В данной статье мне бы хотелось напомнить о проблеме дублирования кода. Не смотря на то, что проблемы, связанные с дублированием кода, упоминаются довольно часто, актуальность этих проблем из года в год остается почти неизменной. Во многих популярных проектах количество клонов измеряется сотнями или тысячами. В рамках данной статьи, мне бы еще раз хотелось напомнить что же такое программные клоны, откуда они берутся, какие влекут за собой проблемы и как с ними можно бороться. В статье приводятся примеры рефакторинга реальных клонов из популярного фреймворка Spring. В качестве инструментов используются **Java 8**,  IDE **IntelliJ IDEA** 2017.1 и плагин **Duplicate Detector** 1.1.

![](imgs/clone_count.png)

### Откуда берутся клоны?

По сути, клоны - это просто схожие фрагменты исходного кода. В основном, они появляются при копировании, даже не смотря на то, что копирование  общеизвестно является плохой практикой. Хотя, это не единственная причина, существуют и другие, более объективные. Например, сам язык программирования может быть недостаточно выразительным, или у разработчика может не хватать прав для редактирования исходного кода.
...
В итоге, такой рефакторинг породит много новых сущностей. Например, в нашем случае это два  класса и один интрефейс. По сути, из-за этого код программы станет только сложнее, поэтому создание собственных wrapper'ов имеет смысл только если подобная ситуация встречается не в первый раз.
...
В этом примере клоны - это конструкторы классов. Такое часто происходит при дублировании самих классов целиком. В этом случае, можно попробовать создать новый общий суперкласс или выразить один из существующих классов через другой.

Подобное наследование иногда также используют и для рефакторинга отдельных фрагментов. Это позволяет в вспомогательных методах получить доступ к полям класса, а сами эти поля исключить из списка явных параметров. С одной стороны, это упрощает сигнатуру метода, однако, с другой стороны, наследование может сильно запутать логику программы, особенно, если оно используется не по своему прямому назначению.
...
* **Используйте возможности среды разработки.** Многие функции среды разработки могут упростить процесс рефакторинга, убрать из него рутину.
